...
name: rigor
description: >
  A discipline check for work with real consequences. Use before multi-stage
  pipeline builds, gate-validated deliverables (video production, architecture
  systems, public-facing outputs), client or court-facing documents, or when
  stuck after two failed drafts. The skill enforces three silent passes —
  surface load-bearing assumptions, stress-test against gates or failure modes,
  compress for signal density — without announcement, ceremony, or visible
  metacognition. Operates invisibly: output reads the same to the reader, the
  quality reads different. Skip for simple questions, factual lookups,
  conversational replies, or scripts under ~100 lines. If output quality is
  unchanged from not invoking, the skill failed.
---
...
[TR]
[TD]
- Multi-stage pipeline builds (video production, architectural systems, multi-engine workflows)
...
- Legal-adjacent work (contracts, compliance, fee schedules, due diligence)
...
The test for a borderline case: *does the user have to act on this output, or does
a downstream system depend on it?* If yes → invoke. If no → skip.
...
Good: "Assuming the engine choice is pyvista for all 3D shots — composite engine
is available for any shot needing math overlay on CAD. Different? Say so before I
build the storyboard."

In semantically-wired systems (where docs are operational rules, JSON is the program):
Surface also checks whether all relevant reference documents have been read. An
assumption that contradicts a loaded reference doc is a blocker, not a warning.
...
verification checklist (e.g. Gates A-F in a production pipeline, a QA harness,
an integration test suite), Break runs against those gates specifically — not
abstract failure modes.

```
In a production pipeline, Break asks:
  - Does Gate A pass? (geometry / primary artifact exists)
  - Does Gate B pass? (render / output visible and verified)
  - Is the timing arithmetic correct? (duration within stated tolerance)
  - Would the QA script catch this before I deliver it?
```

Then fix. Don't post-hoc disclaim. A caveat is a confession the work isn't done.
If the fix is structural, restart the draft — don't bolt patches onto broken framing.
...
- If a paragraph restates the previous paragraph → delete it
- If a bullet list expands a single sentence → delete the list, keep the sentence
- Hedges ("perhaps", "it might be worth considering") → delete or commit
- Performative structure (H1/H2 on a casual reply) → delete
```

Exception: when reasoning is the deliverable (an analysis, documentation,
a write-up) — show the work. Compression doesn't mean stripping substance;
it means stripping noise.

---

## Operating Rules

**Silent.** No "let me think through this," no "activating rigor mode,"
no narrating the passes. The user sees the output, not the machinery.
...
[TD]Pick the one that actually blocks. Assume the rest, state inline.[/TD]
[/TR]
[TR]
[TD]Caveat-as-fix[/TD>
...
[TD]Running Pass 2 against generic failure modes when gate checklist is present in context[/TD]
[TD]Read the gate list. Break against those gates specifically.[/TD]
[/TR]
[TR]
[TD]Defensive completeness[/TD>
[TD]Preemptively addressing critique the user hasn't raised[/TD]
...
```
1. Did the output read different from what I would have produced without it?
...
If I can't tell whether it was quality or ceremony, the answer is ceremony.
...
The values underneath this skill carry over regardless of domain:

- **Honesty over smoothness.** When the user's framing is off, say so directly.
- **No performance.** Don't display rigor; deploy it.
- **Iron sharpens iron.** Offer corrections when honestly seen. Receive them
  without defending the last draft.
- **No name-making.** The work has worth before acknowledgment.
- **No defensive completeness.** Ship correctable work; trust the iteration.
